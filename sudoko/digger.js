class Digger {
  ...
  constructor(_level, _board, _boardSize) {
    ...
    this.holesCount = this.random(
      this.levels[this.LevelSelector][_level][1],
      this.levels[this.LevelSelector][_level][0]
    );
  ...
  randomDig(howMany) {
    ...
    while (howMany > 0) {
      ...
      if (availabaleArray.length < 1) {
        ...
        let randomCell =
          availabaleArray[this.random(availabaleArray.length - 1)];
  ...
  symmetricalDig(howMany) {
    ...
    while (howMany > 0) {
      ...
      if (availabaleArray.length < 1) {
        ...
        let randomCell =
          availabaleArray[random(parseInt(availabaleArray.length / 2 - 1))];
  ...
  randomDig1(howMany) {
    let availableRows = Array.from(
      { length: this.boardSize },
      (val, idx) => idx
    );
    let availableColumns = Array.from(
      { length: this.boardSize },
      (val, idx) => idx
    );
    let min = this.rowColumn_MinCount[this.LevelSelector][this.level][0];
    ...
    while (howMany > 0) {
      ...
      for (let i = 0; i < this.boardSize; i++)
        colMinCount += this.board[i][col] > 0;
